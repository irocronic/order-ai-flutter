...
class _BusinessOwnerHomeState extends State<BusinessOwnerHome>
    ...
    // <<< DEĞİŞİKLİK 1: Sayaçlar ValueNotifier'a dönüştürüldü >>>
    ...
    void dispose() {
        ...
        // <<< DEĞİŞİKLİK 2: Notifier'lar dispose ediliyor. >>>
    ...
    Future<void> _logout() async {
        _socketService.reset(); // <<< DEĞİŞİKLİK: dispose() yerine reset() çağrılıyor >>>
    ...
    Future<void> _fetchActiveOrderCounts() async {
        ...
        try {
            ...
            if (mounted) {
                // <<< DEĞİŞİKLİK 3: setState yerine notifier'ların değerleri güncelleniyor. >>>
    ...
    void _handleSilentOrderUpdates() {
        if (orderStatusUpdateNotifier.value != null && mounted && ModalRoute.of(context)!.isCurrent) {
            _fetchActiveOrderCounts();
        }
    }
    ...
    // <<< DEĞİŞİKLİK 4: _buildIconWithBadge metodu artık ValueListenableBuilder döndürüyor. >>>
    Widget _buildIconWithBadge(IconData defaultIcon, IconData activeIcon, ValueNotifier<int> countNotifier) {
        return ValueListenableBuilder<int>(
            valueListenable: countNotifier,
            builder: (context, count, child) {
                // Hangi sekmenin seçili olduğunu kontrol et
...
void _navigateToKdsScreen(BuildContext context) {
  ...
  if (_availableKdsScreensForUser.length == 1) {
          ...
          content: Container(
            ...
            child: Column(
              ...
              children: [
                ...
                // Flexible widget'ı, ListView'in yüksekliğini belirsiz hale getirdiği için
                // layout hatasına neden oluyordu. Onun yerine yüksekliği sınırlı bir
                // SizedBox kullanarak ListView'e kaydırılabilir bir alan tanıyoruz.
                SizedBox(
                  width: double.maxFinite, // Dialogun genişliğini doldurması için
                  height: MediaQuery.of(context).size.height * 0.3, // Ekran yüksekliğinin %30'u kadar
                  child: ListView.builder(
                    shrinkWrap: true, // Bu hala gerekli
    ...
    // <<< DEĞİŞİKLİK 5: Bu metot artık sayaçlar değiştiğinde yeniden çağrılmıyor. >>>
    void _buildAndSetActiveTabs() {
        ...
        // Bu kontrol, widget ağacında gereksiz değişiklikleri önler.
        ...
        // Sadece index'i güncellemek için ayrı bir kontrol
